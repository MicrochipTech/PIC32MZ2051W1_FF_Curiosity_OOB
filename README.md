...
5. Run the demo and wait till it gets connected to MQTT. Once the connection to MQTT server is established, enter the command "_sysota set 0 0 0 60 "http://192.168.42.95:8000/ota.json"_". This command is to be used to set the correct IP (replace the IP 192.168.42.95 in the above command with the IP of the device where the http server is running).

      <p align="center">
      <img src="resources/media/sysota_set.png"/>
      <p/>
  
6. Enter the command "_start ota_". This command will trigger the OTA demo and download the test app binary from the server.

      <p align="center">
      <img src="resources/media/start_ota.png"/>
      <p/>
  
7. After completing the download, the device will have a reset and ask the user to "_Press 'a' for Cloud Image or 'b' for OTA Downloaded Image_". 
...
1. Connect the [QT8 Xplained Pro](https://www.microchip.com/en-us/development-tool/ac164161) to the Xpro Header of the curiosity board.
...
3. When the demo starts, all the leds on the [QT8 Xplained Pro](https://www.microchip.com/en-us/development-tool/ac164161) will be glowing. After enabling the touch functionality , the leds turn off and start glowing based on the touch position.
...
2.  Open the project in MPLABX.
      ...
      - Use [MPLABX](https://www.microchip.com/mplab/mplab-x-ide?gclid=Cj0KCQjw_ez2BRCyARIsAJfg-ksTefWxHYsG43Succ4obrD2ecwpP99wMUHjUCuoBdvmveCIB8JgoMIaAiCdEALw_wcB) version `6.20` or above and [XC32](https://www.microchip.com/mplab/compilers) version `4.35`.
...
4.  Recompile the application and download it into the target.
    ...
    - To compile the project, open the firmware using [MPLAB X IDE](https://www.microchip.com/mplab/mplab-x-ide) version `6.20` or higher and use [XC32](https://www.microchip.com/mplab/compilers) compiler version `4.35` or higher
