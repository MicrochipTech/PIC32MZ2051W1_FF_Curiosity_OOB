...
void _APP_Commands_Start(SYS_CMD_DEVICE_NODE* pCmdIO, int argc, char** argv);
...
static const SYS_CMD_DESCRIPTOR appCmdTbl[] = {
    ...
    {"start", _APP_Commands_Start, ": Start Cloud or OTA"},
...
//    const void* cmdIoParam = pCmdIO->cmdIoParam;
    ...
    if ((argc == 2) && (!strcmp((const char*)argv[1], "cloud"))) 
    {
        APP_SetMode(APP_CLOUD);
    }
    if ((argc == 2) && (!strcmp((const char*)argv[1], "ota"))) 
    {
        APP_SetMode(APP_OTA);
    }
    else 
