...
bool APP_WIFI_ConnectedState(void)
{
   return  app_wifiData.isConnected;
}


void APP_WiFiServCallback(uint32_t event, void * data, void *cookie) 
{
    switch (event) 
    {
...
void APP_WIFI_Tasks(void) {
 ...
 switch (app_wifiData.state) {
        case APP_WIFI_STATE_INIT:
            ret=SYS_WIFI_CtrlMsg(sysObj.syswifi,SYS_WIFI_REGCALLBACK,APP_WiFiServCallback,sizeof(uint8_t*));
